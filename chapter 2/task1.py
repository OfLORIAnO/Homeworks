...
def task1() -> str:
    """
    Функция запрашивает строку у пользователя, определяет самую длинную подстроку,
    не содержащую повторяющихся символов, и возвращает её

    Return:
    str: Самая длинная подстрока без повторяющихся символов
    """
